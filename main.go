...
func generateMockType0(w io.Writer, mocktag string, pkgname string, typ *srcdom.Type, pkg *srcdom.Package) error {
	...
	fmt.Fprintf(w, "// %s is a mock for %s.%[1]s\n", typ.Name, pkg.Name)
	...
	for _, m := range methods {
		...
		fmt.Fprintf(w, "}\n\n")
		fmt.Fprintf(w, "// %s wraps %s#%[1]s method\n", m.name, origtyp)
		fmt.Fprintf(w, "func (_m *%s) %s(%s) (%s) {\n", typ.Name, m.name, m.args.nameTypes(), m.rets.types())
		fmt.Fprintf(w, "\t_m.%[2]s_Ps = append(_m.%[2]s_Ps, &%[1]s%[2]s{%s})\n", typ.Name, m.name, m.args.names())
		fmt.Fprintf(w, "\tvar r *%[1]s%[2]s_R\n", typ.Name, m.name)
		fmt.Fprintf(w, "\t_v, _m.%[1]s_Rs = _m.%[1]s_Rs[0], _m.%[1]s_Rs[1:]\n", m.name)
		fmt.Fprintf(w, "\treturn %s\n", m.rets.namesPrefix("_v"))
		fmt.Fprintf(w, "}\n")
	}
	return nil
}
